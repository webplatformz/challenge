...
let ClientApi = {
    ...
    broadcastCardPlayed : function broadcastCardPlayed(playedCards) {
        this.clients.forEach((client) => {
            client.send(toJSON(messages.create(messages.MessageType.CARDS_PLAYED, playedCards)));
        });
    },

    broadcastTrumpf : function broadcastTrumpf(gameType) {
        this.clients.forEach((client) => {
            client.send(toJSON(messages.create(messages.MessageType.BROADCAST_TRUMPF, gameType)));
        });
    }
};
