...
let Game = {
    ...
    nextCycle: function (round) {
        let onCycleFinished = (playedCards) => {
            if(round === 9) {
                // fertig
                console.log("finish");
            } else {

                this.nextCycle(++round);
            }
        };

        let cycle = Cycle.create(this.startPlayer, this.players, this.clientApi, onCycleFinished);
...
let dealCards = function dealCards(cards, players) {
    players[0].cards = cards.splice(0, 9);
    players[1].cards = cards.splice(0, 9);
    players[2].cards = cards.splice(0, 9);
    players[3].cards = cards.splice(0, 9);
};

let create = function create(players, maxPoints, startPlayer, clientApi) {
    let game = Object.create(Game);
    game.deck = Deck.create();
    dealCards(game.deck.cards, players);
